...
class BoundaryExploration(Node):
    ...
    def exploration(self, 
        ...
        if len(groups) == 0:
            ...
            if path != None:
                ...
                path = -1
        if self.is_fully_enclosed(map_occupancy_data, (column, row)):
            print("[INFO] AREA IS FULLY ENCLOSED -> FINISHING EXPLORATION")
            path = -1
    ...
    def is_fully_enclosed(
        ...
        rande: int = 1   # neighborhood radius for adjacency tests (kept at 1)
    ) -> bool:
        """
        The robot is inside a fully known enclosure iff:
        - From its FREE cell, flood-filling through FREE cells cannot reach the map boundary, AND
        - No cell in the reachable FREE region has an UNKNOWN (-1) neighbor (the boundary is 'fully known').

        Returns True if enclosed, else False.
        """
        rows, cols = grid.shape
        r0, c0 = position

        # Start must be inside the map and on FREE
        if not (0 <= r0 < rows and 0 <= c0 < cols):
            return False
        if grid[r0, c0] != free_val:
            return False

        # 8-connected movement; also use 8-neighborhood for "unknown adjacency" checks
        ...
        while q:
            ...
            # If any reachable FREE cell touches the outer boundary → not enclosed
            if x == 0 or y == 0 or x == rows - 1 or y == cols - 1:
                return False

            # If any UNKNOWN neighbors the reachable FREE region → boundary is NOT fully known → not enclosed
            for dx, dy in directions:
                nx, ny = x + dx, y + dy
            ...
            # Continue flooding through FREE cells
            for dx, dy in directions:
                nx, ny = x + dx, y + dy
        ...
        # Finished without reaching border and without seeing UNKNOWN along the boundary → enclosed
        return True
