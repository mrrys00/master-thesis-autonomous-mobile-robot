...
# -------------------- PATH PLANNING (A*) --------------------
def astar(array: list[float], start: tuple[int], goal: tuple[int]) -> list[tuple[int]] | bool:
...
# -------------------- B-SPLINE --------------------
def bspline_planning(array: list[tuple], sn: int) -> list[tuple]:
    try:
...
# -------------------- FRONTIER DETECTION --------------------
def frontierB(matrix: list[float]) -> list[float]:
...
# -------------------- GROUPING --------------------
def assign_groups(matrix: list[float]) -> tuple[list[float], dict[int, list[tuple[int, int]]]]:
    group = 1
    groups = {}
    for i in range(len(matrix)):
...
# -------------------- OBSTACLE CLUSTERING --------------------
# TODO: merge with frontier grouping
def dfs_obstacle(
        matrix: list[float],
        i: int,
        j: int,
...
def assign_obstacle_groups(matrix: list[float]) -> dict[int, list[tuple[int, int]]]:
    group = 1
    groups = {}
    for i in range(len(matrix)):
        for j in range(len(matrix[0])):
...
# -------------------- FRONTIER SELECTION --------------------
def fGroups(groups: dict[int, list[tuple[int, int]]]) -> list[tuple[int, list[tuple[int, int]]]]:
    sorted_groups = sorted(groups.items(), key=lambda x: len(x[1]), reverse=True)
    return sorted_groups

def calculate_centroid(x_coords: list[int], y_coords: list[int]) -> tuple[int, int]:
    n = len(x_coords)
...
def costmap(
    ...
    wall = np.where(map_occupancy_data == 100)
...
def exploration(
    ...
    map_occupancy_data = frontierB(map_occupancy_data)
    map_occupancy_data, groups = assign_groups(map_occupancy_data)
...
# -------------------- LOCAL CONTROL --------------------
def localControl(scan) -> tuple[float,float | None,None]:
    v = None
    w = None
    for i in range(60):
        if scan[i] < robot_r:
        ...
        while True:
            ...
            if self.target == True:
                if isinstance(pathGlobal, int) and pathGlobal == 0:
                    ...
                    exploration(self.map_occupancy_data, 
                        self.width, 
                        self.height, 
                        self.resolution, 
                        column, 
